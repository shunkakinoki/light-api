...
const getHeaders = (
  nameOrHeaders?: RateLimitHeaders,
...
const rateLimit = async (context: RateLimitContext): Promise<Response> => {
...
export const initRateLimit = (fn: RateLimitHandler) => {
  return async function isRateLimited(request: NextRequest): Promise<Response> {
